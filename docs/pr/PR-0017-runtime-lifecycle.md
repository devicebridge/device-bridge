...
Runtime Device Bridge получил управляемый жизненный цикл. При отмене контекста все Sources корректно завершаются, Bus закрывается, goroutine не зависают. Ошибки Sources больше не теряются.
